...
class Bilinear:
    vertices = TwoPointFn.vertices
    currents = ["S", "P", "V", "A", "T"]

    def __init__(
        self, ensemble: str, scheme: str = "SMOM", compute: bool = False
    ) -> None:
    ...
    def plot_chiral_extrap_allmomvar(self, subscheme: str) -> None:
        fig, ax = plt.subplots(nrows=len(self.currents), ncols=1, figsize=(3, 10))
    ...
    def load_chiral_extrap(
        self, momvar_idx: int, subscheme: str, plot: bool = False
    ) -> Dict:
        ...
        grp_name = f"Bilinear/m0p0/{subscheme}" + f"/momvar_{momvar_idx+1}"
        grp = file[grp_name]
    ...
    def chiral_extrap(
        ...
        if save:
            ...
            for current in self.currents:
                grp.create_dataset(f"{current}/central", data=extrap[current].val)
                grp.create_dataset(f"{current}/errors", data=extrap[current].err)
                grp.create_dataset(f"{current}/bootstrap", data=extrap[current].btsp)
    ...
    def get_Z_all_masses(
        self, momvar_idx: int, subscheme: str, plot: bool = True
    ) -> Dict:

        Zs = {
            mass: self.get_Z_all_mom(mass, momvar_idx, subscheme)
    ...
    def plot_Z_factors(self, Zs: Dict, title: str, fname: str) -> None:
        fig, ax = plt.subplots(
            nrows=2, sharex=True, figsize=(3, 5), gridspec_kw={"height_ratios": [2, 1]}
        )
        plt.subplots_adjust(hspace=0)
        ...
        for X, Y in zip(["S", "V"], ["P", "A"]):
            ys = Zs[X] / Zs[Y]
            ax[1].errorbar(
                self.momenta, ys.val, ys.err, fmt="o", capsize=4, label=f"{X}/{Y}"
            )
    ...
    def project_vertices(
        ...
        operators = self.construct_operators(mass, mom, theta_in, theta_out)
        ...
        return {
            current: sum(
                [
                    projectors[current][i] @ operators[current][i]
                    for i in range(len(operators[current]))
                ],
                Zero(operators[current][0].shape),
            )
            .use_func(np.trace)
            .use_func(np.real)
    ...
    def construct_operators(
        ...
        amputees = self.amputate_bilinears(mass, mom, theta_in, theta_out)
        ...
        return {
            ...
            "T": sum(
                [
                    [
                        amputees[self.vertices.index(f"Sigma{dirs[mu]}{dirs[nu]}")]
    ...
    def amputate_bilinears(
        self, mass: float, mom: float, theta_in: np.ndarray, theta_out: np.ndarray
    ) -> np.ndarray:

        bilinears = self.read_in_bilinears(mass, mom, theta_in, theta_out)
        ...
        return np.array(
            [
                out_prop_inv @ bilinears[b] @ in_prop_inv
                for b in range(len(self.vertices))
            ]
        )

    def read_in_bilinears(
        self, mass: float, mom: float, theta_in: np.ndarray, theta_out: np.ndarray
    ) -> np.ndarray:
        ...
        data = np.empty(
            shape=(self.N_cf, len(self.vertices), N_cd, N_cd), dtype="complex128"
        )
        ...
        bilinears = np.array(
            [
                ...
                for b_idx in range(len(self.vertices))
            ],
            dtype=object,
        )
    ...
    def list_bilinears(self, path: str) -> np.ndarray:
    ...
    def create_attributes(self) -> None:
        self.theta_str = {
            mass_str: {
                mom_str: self.list_bilinears(f"{self.path}/{mass_str}/{mom_str}")
...
def bilinear_projectors(subscheme: str, qvec: np.ndarray) -> Dict:
    ...
    projectors = {
        ...
        "T": sum(
            [
                [
                    myGamma[dirs[i]] @ myGamma[dirs[j]]
                    if subscheme == "gamma"
                    else (
                        myGamma[dirs[j]] @ Gamma[dirs[i]]
                        - myGamma[dirs[i]] @ Gamma[dirs[j]]
                    )
    ...
    tree_values = {
        curr: np.sum([np.trace(mtx @ mtx) for mtx in proj], axis=0)
        for curr, proj in projectors.items()
    }

    return {
        curr: [Stat(val=mtx / tree_values[curr], btsp="constant") for mtx in proj]
        for curr, proj in projectors.items()
    }
